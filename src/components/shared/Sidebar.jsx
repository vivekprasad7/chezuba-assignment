...
const navStyles = ({isActive}) => ({
    backgroundColor: isActive ? "rgb(240 249 255)" : "",
...
const Sidebar = () => {
  ...
  return (
    <div className={`hidden transition-all duration-300 ${showSidebar ? "w-60" : "w-20"} md:flex md:flex-col  p-2 py-3 shadow-lg h-full`}>
        ...
        <FcDoughnutChart  className={` transition-all duration-300 ${ showSidebar ? "" : "w-0"}`} fontSize={36} />
        <h1 className={`overflow-hidden transition-all ${ showSidebar ? "px-2 text-2xl w-32" : "w-0"}`}>Chezuba</h1>
            </div>
...
function SidebarLink({ link, showSidebar}) {
    return (
      <NavLink
        to={link.path}
        style={navStyles}
        className={`relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer hover:bg-sky-50
       `}
      >
